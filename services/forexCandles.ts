...
/** Yahoo chart path must keep `=` in symbols (e.g. EURUSD=X). `encodeURIComponent` turns `=` into `%3D` and breaks requests. */
...
export async function fetchYahooForexCandles(
  ...
  const cached = getYahooCached(pairId, resolution);
  ...
  const inflightKey = yahooCacheKey(pairId, resolution);
  ...
  const promise = (async (): Promise<{ bars: OhlcBar[]; lastError?: string }> => {
    ...
    try {
      ...
      if (result.rateLimited) return yahooDiskOrCooldown(pairId, resolution, lastErr);
      if (result.bars.length > 0) {
        setYahooCached(pairId, resolution, result.bars);
      ...
      return yahooDiskOrCooldown(pairId, resolution, emptyMsg);
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'fetch failed';
      return yahooDiskOrCooldown(pairId, resolution, msg);
    }
  })();
