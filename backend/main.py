...
class CommodityForecastBody(BaseModel):
    commodity_id: str = Field(..., min_length=2, max_length=8)
    ...
    use_csv_sentiment: bool = Field(
        default=True,
        description="When true, fill missing sentiment from daily_sentiment_features.csv",
    )


@app.get("/api/commodities/list")
def get_commodities_list() -> dict[str, Any]:
    return {"commodities": list_commodity_ids()}


@app.post("/api/commodities/forecast")
def post_commodity_forecast(body: CommodityForecastBody) -> dict[str, Any]:
    """
    Train/load daily 3-state HMM per commodity OHLC; return expected price and
    support/resistance/sideways probabilities for 1d, 5d, 1m, 3m, 1y, 3y horizons.
    """
    try:
        payload = build_commodity_forecast_payload(
            body.commodity_id,
            sentiment_score=body.sentiment_score,
            sentiment_confidence=body.sentiment_confidence,
            use_csv_sentiment=body.use_csv_sentiment,
        )
        return jsonable_encoder(payload)
    except FileNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e)) from e
...
def get_macro_daily() -> dict[str, Any]:
    """US macro indicators from backend/data/macro_daily.csv (FRED snapshot)."""
    try:
        return jsonable_encoder(load_macro_daily())
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to read macro CSV: {e}") from e
...
def get_macro_report(
    ...
    try:
        ...
        if payload is None:
            return jsonable_encoder({"report_date": day, "found": False, "content": None, "path": None})
...
class MacroReportSaveBody(BaseModel):
    report_date: str = Field(..., min_length=10, max_length=10, description="YYYY-MM-DD")
    content: str = Field(..., min_length=1)


@app.post("/api/macro/report")
def post_macro_report(body: MacroReportSaveBody) -> dict[str, Any]:
    """Save macro LLM report for a calendar day (one file per day)."""
    try:
        return jsonable_encoder(save_macro_report(body.report_date.strip(), body.content))
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e)) from e
    except Exception as e:
