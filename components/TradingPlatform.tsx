...
import CommodityHMMForecast from './CommodityHMMForecast';
...
const TradingPlatform: React.FC<{ isPaperTrader?: boolean }> = ({ isPaperTrader }) => {
  /** Full S&P, forex, crypto, and commodities — always available here regardless of `VITE_SP500_ONLY` (that flag still scopes other areas). */
  const panelNavItems = PANEL_NAV;
  ...
  const [commodityCandleSource, setCommodityCandleSource] = useState<'finnhub' | 'yahoo' | 'local' | 'none' | null>(
    null
  );
  const [commodityCandleDetail, setCommodityCandleDetail] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (panel !== 'commodities' || !selectedCommodity) return;

    let cancelled = false;
    const yahooSym = selectedCommodity.finnhub;

    const loadCandles = async (reset: boolean) => {
      if (reset) {
        setCommodityCandles([]);
        setCommodityCandleLoading(true);
        setCommodityCandleError(null);
        setCommodityCandleDetail(null);
      }
      try {
        const { bars, source, detail } = await fetchCommodityDailyCandlesWithFallback(
          selectedCommodity.id,
          yahooSym
        );
        if (!cancelled) {
          setCommodityCandles(bars);
          setCommodityCandleSource(source);
          setCommodityCandleDetail(detail ?? null);
        }
      } catch (e: unknown) {
        if (!cancelled && reset) {
          setCommodityCandleError(e instanceof Error ? e.message : 'Could not load commodity candles');
        }
      } finally {
        if (!cancelled && reset) setCommodityCandleLoading(false);
      }
    };

    void loadCandles(true);
    const tick = window.setInterval(() => void loadCandles(false), 24 * 60 * 60_000);

    return () => {
      cancelled = true;
      window.clearInterval(tick);
    };
  }, [panel, selectedCommodity]);
      ...
      <main className="flex-1 min-w-0 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              ...
              {panel === 'commodities' && 'Metals & oil futures (COMEX / NYMEX-style symbols) via Finnhub'}
            </p>
          </div>
          {lastUpdated ? (
        ...
        {panel === 'commodities' && (
          <div className="rounded-2xl border border-amber-100 bg-gradient-to-br from-amber-50/30 to-white shadow-sm overflow-hidden">
            <div className="px-4 pt-4 pb-2 md:px-5 md:pt-5 flex flex-wrap items-center justify-between gap-2">
              <h3 className="text-sm font-black uppercase tracking-widest text-amber-950">Commodities</h3>
              {quotesLoading ? <span className="text-xs text-amber-700/80">Refreshing…</span> : null}
            </div>
            {!finnhubKey ? (
              <p className="px-4 md:px-5 pb-2 text-xs text-amber-800">
                Add <code className="bg-amber-100 px-1 rounded">VITE_FINNHUB_KEY</code> for live quote tiles. Daily
                charts use saved CSV or Yahoo.
              </p>
            ) : null}
            <div className="flex flex-nowrap items-stretch gap-2 overflow-x-auto px-4 pb-3 md:px-5">
              {METAL_QUOTE_ROWS.map((row) => {
                const price = metalPrices[row.id];
                const selected = paperMetalId === row.id;
                return (
                  <button
                    type="button"
                    key={row.id}
                    onClick={() => setPaperMetalId(row.id)}
                    aria-pressed={selected}
                    className={`shrink-0 rounded-lg border bg-white px-2 py-1.5 min-w-[4.85rem] max-w-[6.25rem] flex flex-col items-center text-center shadow-sm transition-shadow ${
                      selected
                        ? 'border-amber-600 ring-2 ring-amber-400/50 ring-offset-1'
            ...
            <div className="mx-4 mb-3 md:mx-5 rounded-xl border border-amber-100 bg-gradient-to-r from-amber-50/80 to-white px-3 py-3 shadow-inner">
              <p className="text-[10px] font-black uppercase tracking-widest text-amber-800 mb-2">
                Market simulation · {selectedCommodity?.label ?? paperMetalId}
              </p>
              <PaperTradeBar
                userId={simUserId}
                kind="metal"
                symbol={paperMetalId}
                assetLabel={selectedCommodity?.label ?? paperMetalId}
            ...
            <div className="mx-4 mb-4 md:mx-5 flex flex-col gap-3 rounded-xl border border-dashed border-amber-200/80 bg-amber-50/30 p-3 min-h-[320px] md:min-h-[380px]">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[10px] font-black uppercase tracking-widest text-amber-700/90">Daily chart</span>
                {commodityCandleSource ? (
                  <span className="text-[10px] font-semibold text-amber-800/80 uppercase">{commodityCandleSource}</span>
                ) : null}
              </div>
              <div className="flex-1 flex flex-col min-h-[280px]">
                <ForexCandleChart
                  bars={commodityCandles}
                  loading={commodityCandleLoading}
                  error={commodityCandleError}
                  dataSource={commodityCandleSource}
                  diagnostic={commodityCandleDetail}
                  resolutionId="D"
                  forexSymbol={selectedCommodity?.id ?? paperMetalId}
                  pricePrecision={2}
                  priceMinMove={0.01}
                  pairLabel={selectedCommodity?.label ?? paperMetalId}
                />
              </div>
            </div>

            <CommodityHMMForecast
              commodityId={selectedCommodity?.id ?? paperMetalId}
              commodityLabel={selectedCommodity?.label}
            />
          </div>
        )}
      </main>
